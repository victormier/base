...
There's a known problem with translations related routes if translations table is not created. Just comment these lines, apply the migrations, and uncomment again:
...
There's a `deploy` rake task that deploys to heroku. Still not much tested. For setting it up you have to change the following lines in `lib/tasks/deploy.rake` (just the production one if you don't have a stagin environment):

  PRODUCTION_APP = 'YOUR_PRODUCTION_APP_NAME_ON_HEROKU'
  STAGING_APP = 'YOUR_STAGING_APP_NAME_ON_HEROKU'

It marks deploys and releases and there are several tasks. The main one is:
